...
def conv(image, label, params, gamma, offset, validation = False):
    ...
    probs = softmaxBatch(measured/offset) # predict class probabilities with the softmax activation function
...
def adamGD(batch, num_classes, lr, dim, n_c, beta1, beta2, params, cost, gamma, offset):
    '''
    ...
    x = X
    y = offset + np.eye(num_classes)[Y.astype(int)].reshape(batch_size, num_classes, 1) # convert label to one-hot
    ...
    grads, loss, nl1, nl2, nl3 = conv(x, y, params, gamma, offset)
...
def train(num_classes = 3, lr = 0.01, beta1 = 0.95, beta2 = 0.99, offset=10,
    ...
    train_data = norm_stack_shuffle(X,y_dash, offset, by_column=False)

    val_data = norm_stack_shuffle(X_val,y_dash_val, offset, by_column=False)
    ...
    if not continue_training:
        ...
        f1 = initializeFilter(f1, offset)
        f2 = initializeFilter(f2, offset)
        w3 = initializeFilter(w3, offset)
        w4 = initializeWeight(w4, offset)
        ...
        b1 = initializeBias(b1, offset)
        b2 = initializeBias(b2, offset)
        b3 = initializeBias(b3, offset)
        b4 = initializeBias(b4, offset)
    ...
    for epoch in t:
        ...
        y_val = offset + np.eye(num_classes)[Y_val.astype(int)].reshape(-1, num_classes, 1) # convert label to one-hot

        c_val = conv(x_val, y_val, params, gamma, offset, validation=True)
        ...
        for batch in batches:
            ...
            params, cost, nl1, nl2, nl3 = adamGD(batch, num_classes, lr, img_dim, img_depth, beta1, beta2, params, cost, gamma, offset)
