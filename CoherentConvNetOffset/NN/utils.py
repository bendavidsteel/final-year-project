...
def initializeFilter(size, offset, scale = 1.0):
...
def initializeWeight(size, offset):
    # scale by inverse square root of size of previous layer
    var = np.sqrt(6)/np.sqrt(size[0] + size[1])
    return np.random.uniform(low = -var, high = var, size = size) + 1j*np.random.uniform(low = -var, high = var, size = size)

def initializeBias(size, offset):
	var = 1
	# return np.random.uniform(low = 0, high = var, size = size)
	return offset + np.random.uniform(low = -var, high = var, size = size) + 1j*(offset + np.random.uniform(low = -var, high = var, size = size))
...
def norm_stack_shuffle(x, y_dash, offset, by_column=True):
...
def predict(image, label, params, gamma, offset):
	'''
