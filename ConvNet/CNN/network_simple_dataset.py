...
def conv(image, label, params, config):
    ...
    pooled = lorentz(conv2, b2.reshape(num_filt2, 1, 1), gamma) # pass through Lorentzian non-linearity
    
    (nf2, dim2, _) = pooled.shape
    fc = pooled.reshape((nf2 * dim2 * dim2, 1)) # flatten pooled layer
    
    z1 = w3.dot(fc)
    a1 = lorentz(z1, b3, gamma)

    # z2 = w4.dot(a1) # first dense layer
    # a2 = lorentz(z2, b4, gamma) # pass through Lorentzian non-linearity
    
    # out = w5.dot(a2) + b5 # second dense layer
    ...
    # dw5 = dout * a2.T # loss gradient of final dense layer weights
    # db5 = dout # loss gradient of final dense layer biases
    
    # da2 = w5.T.dot(dout) # loss gradient of first dense layer outputs 

    # dl4 = lorentzDx(z2, b4, gamma)

    # dw4 = da2 * dl4 * a1.T
    # db4 = da2 * lorentzDx0(z2, b4, gamma)
    
    # da1 = w4.T.dot(da2 * dl4) # loss gradients of fully-connected layer
    ...
    dl3 = lorentzDx(z1, b3, gamma)

    dw3 = da1 * dl3 * fc.T
    db3 = da1 * lorentzDx0(z1, b3, gamma)

    dfc = w3.T.dot(da1 * dl3)
    dpool = dfc.reshape(pooled.shape) # reshape fully connected into dimensions of pooling layer
    
    dconv2 = dpool * lorentzDx(conv2, b2.reshape(num_filt2, 1, 1), gamma) # backpropagate through lorentzian
    db2 = np.mean(dpool * lorentzDx0(conv2, b2.reshape(num_filt2, 1, 1), gamma), axis=(1,2)) # find grad for bias
