...
def network(batch, label, params, gamma, validation = False):
    ...
    ############## Forward Operation ###############
    ################################################
    z1 = np.matmul(np.square(w1), np.square(batch)) # convolution operation
    a1 = lorentz(z1, b1.reshape(1,-1,1), gamma[0]) # pass through Lorentzian non-linearity
    
    z2 = np.matmul(np.square(w2), np.square(a1)) # second convolution operation
    ...
    out = np.square(np.matmul(np.square(w3), np.square(a2))) + np.square(b3.reshape(1,-1,1))
