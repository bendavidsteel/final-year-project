...
def convolutionLorentzBatch(image, filt, gamma, s=1):
    '''
    Confolves `filt` over `image` using stride `s`
    '''
    (n_f, n_c_f, f, _) = filt.shape # filter dimensions
    ...
    for curr_b in range(batch_size):
        for curr_f in range(n_f):
            ...
            while curr_y + f <= in_dim:
                curr_x = out_x = 0
                while curr_x + f <= in_dim:
                    out[curr_b, curr_f, out_y, out_x] = np.sum(lorentz(image[curr_b, :, curr_y:curr_y+f, curr_x:curr_x+f], filt[curr_f], gamma))
                    curr_x += s
                    out_x += 1
                curr_y += s
                out_y += 1
...
def convolutionLorentzBatch2(image, filt, gamma, s=1):
    '''
    Confolves `filt` over `image` using stride `s`
    '''
    (n_f, n_c_f, f, _) = filt.shape # filter dimensions
    ...
    out = np.zeros((batch_size, n_f,out_dim,out_dim))
    nl_store = np.zeros((batch_size, n_f, (in_dim - f + 1)//s, (in_dim - f + 1)//s, f, f))
    ...
    for curr_b in range(batch_size):
        for curr_f in range(n_f):
            ...
            while curr_y + f <= in_dim:
                curr_x = out_x = 0
                while curr_x + f <= in_dim:
                    nl = lorentz(image[curr_b, :, curr_y:curr_y+f, curr_x:curr_x+f], filt[curr_f], gamma)
                    nl_store[curr_b, curr_f, curr_y, curr_x, :, :] = nl[0]
                    out[curr_b, curr_f, out_y, out_x] = np.sum(nl)
                    curr_x += s
                    out_x += 1
                curr_y += s
                out_y += 1
